...
public class AppBulkTool implements CommandLineRunner, ExitCodeGenerator {
        ...
        switch (args[0]) {
            case "--registerCustomer" -> {
                RegisterRequest request = new RegisterRequest("Jürgen", "Becker", null, "ralf@test.de", "01702934959",
                        ...
                        List.of(AppConstants.PRODUCT_CRM_UPLOAD, AppConstants.PRODUCT_DUPLICATE_CHECK),true,true,true,"21.12.2025");
    ...
    private void processLexware(TASK task) throws Exception {
        ...
        for (int i = 1; i <= nFiles; i++) {
            ...
            LexwareGenerateWorkbookDuplicateCheck.createWorkbook(filename, 100, String.format("LW-%04d", i));
            if (task.equals(TASK.TO_ESPO)) {
