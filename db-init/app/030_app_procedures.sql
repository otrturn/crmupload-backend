...
AS $$
WITH price_map AS (
    SELECT 1 AS product_count, 250 AS price
    UNION ALL
    SELECT 2, 400
),
     customer_open_products AS (
         SELECT
             cp.customer_id,
...
SELECT
    COALESCE(SUM(CASE WHEN product_count = 1 THEN 1 ELSE 0 END), 0)::bigint AS customers_1_product,
