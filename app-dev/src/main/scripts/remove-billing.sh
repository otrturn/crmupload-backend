...
# Alle Container stoppen, die auf dem Image "crmupload-billing" basieren
...
# Alle Images löschen, die "crmupload-billing" enthalten
echo "🧹 Entferne Images mit Namen 'crmupload-web'..."
docker images --format "{{.Repository}}:{{.Tag}} {{.ID}}" | grep 'crmupload-billing' | while read repo id; do
	if [ -n "$id" ]; then
